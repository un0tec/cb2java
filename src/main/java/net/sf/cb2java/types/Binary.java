...
public class Binary extends Numeric {
    ...
    public Data parse(InputStream input) throws IOException {
        ...
        BigInteger bigI = new BigInteger(readNextBytesFromStream(input, this.getLength()));
...
//        display(copybook.values.LOW_VALUES.fill(bigI.toByteArray(), length(), Value.LEFT));
    ...
    public static class Native extends Binary {
        ...
        public Data parse(InputStream inputStream) throws IOException {
            // TODO return super.parse(getSettings().getLittleEndian() ? reverse(inputStream) : inputStream);
